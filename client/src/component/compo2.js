...
function Compo2() {
    ...
    const handleSubmit = (event) => {
        ...
        if (fname.length !== 0 && lname.length !== 0 && email.length !== 0 && addres.length !== 0 && pincode.length !== 0) {
            event.preventDefault();
            axios.post('https://sa-mern-user.onrender.com/creatuser', { fname: full, email: email, mobile: mobile, address: addres, address2: add2, country: country, state: state, pincode: pincode })
            clearInputFields();
            alert("Submited");
        }
    };
